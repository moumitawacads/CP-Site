  ...
  <div class="col-md-6">
    <div class="card px-4 py-4">
      <h2 class="text-3xl font-bold text-center mb-6">
        {{ $t("learner_registration") }}
      </h2>
      <form class="needs-validation" novalidate ref="learnerRegisterForm" @submit.prevent="handleSubmit">
        ...
        <div class="form-group mt-2">
          <label for="culture">{{ $t("culture") }}:</label>
          <select v-model="formData.culture" required class="form-control">
        ...
        <div class="form-group mt-2">
          <label for="learner_email">{{ $t("email") }}:</label>
          <input type="email" id="learner_email" class="form-control" v-model="formData.learner_email"
            autocomplete="off" required />
          <div class="invalid-feedback">{{ $t(errors.learner_email) }}</div>
        </div>
        <div class="form-group mt-2">
          <label for="learner_phone_number">{{ $t("phone_number") }}:</label>
...
export default {
  ...
  methods: {
    async handleSubmit(event) {
      ...
      if (validation) {
        try {
          const response = await axios.post("/api/learner/register", this.formData);
