  ...
  <div class="col-md-6">
    <div class="card px-4 py-4">
      <h2 class="text-3xl font-bold text-center mb-6">
        {{ $t("learner_registration_by_parent") }}
      </h2>
      <form
        class="needs-validation"
        novalidate
        ref="learnerRegistrationByParentForm"
      ...
      >
        ...
        <div class="form-group mt-2">
          ...
          <input
            ...
            id="name_of_school"
            class="form-control"
            v-model="formData.name_of_school"
            required
          />
        ...
        <div class="form-group mt-2">
          ...
          <input
            ...
            id="learner_email"
            class="form-control"
            v-model="formData.learner_email"
            required
          />
        ...
        <div class="form-group mt-2">
          ...
          <input
            ...
            class="form-control"
            v-model="formData.learner_phone_number"
            required
          />
        ...
        <div class="form-group mt-2">
          ...
          <input
            ...
            id="family_diagnosis"
            class="form-control"
            v-model="formData.family_diagnosis"
            required
          />
        ...
        <div class="form-group mt-2">
          ...
          <input
            ...
            id="learner_type"
            class="form-control"
            v-model="formData.learner_type"
            required
          />
          <div class="invalid-feedback">{{ $t(errors.learner_type) }}</div>
        </div>
        <div class="form-group mt-2">
          <label for="speech_language_diagnosis"
            >{{ $t("speech_language_diagnosis") }}:</label
          >
          <select
            v-model="formData.speech_language_diagnosis"
            required
            class="form-select"
          >
            <option value="">{{ $t("select_any") }}</option>
            <option
              v-for="(speechLanguageDiagnosis, index) in speechLanguageDiagnosisList"
              :key="index + 1"
...
export default {
  ...
  mounted() {
    ...
    this.parentData = this.$ls.get("parent_data");
    this.formData.parent_id = this.parentData.id;
  },
  methods: {
      ...
      if (validation) {
        try {
          const response = await axios.post(
            "/api/parents/learner-registration-by-parents",
            this.formData
          );
          ...
          if (response && response.status == 200) {
            ...
            this.$ls.set("token", response.data.user.access_token);
            this.$ls.set("token_expiry", expiresAt);
            this.$ls.set("user", response.data.user);
            this.$ls.set("whole_data", response.data);
            this.$ls.set("learner_id", response.data.user.learner_id);
            this.$ls.set(
              "learner_encrypt_data_id",
              response.data.user.learner_encrypt_data_id
            );
            this.$router.push("/clinician-learner-link");
          }
        } catch (error) {
          if (
